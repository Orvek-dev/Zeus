...
`v1.0.0-rc` is a governed Live Beta Candidate source checkpoint. It includes deterministic local scenarios, provider and tool interfaces, native tool catalog reporting, MCP discovery and API connector dry-run contracts, CLI/API/gateway/ACP/batch/Python-library entrypoint contracts, local MemoryGraph storage, LLM Wiki views, ontology review queues, skill-learning memory bridges, retention-policy reporting, adaptive ULW workflow selection, critique checkpoints, live readiness, local opt-in smoke, live cockpit reporting, live beta provider/MCP/gateway contracts, dynamic workflow contracts, transport state, gateway/workflow scaffolds, verification surfaces, security planning, runtime lease scope checks, research evidence graph contracts, ontology candidate controls, sandbox workflow hints, live opt-in boundaries, public/private release boundaries, and release-gated hard-close reporting. Memory/Ontology status commands may initialize the local SQLite MemoryGraph schema under the selected Zeus home. Adaptive Zeus status commands only plan dry-run workflow patterns. Live Beta Candidate status commands only compose readiness and local smoke evidence. They do not open network access, run production handlers, self-modify workflows, auto-write memory, promote ontology terms, promote learned rules, or write active rules. This release candidate is not a hard-isolated remote execution platform by itself.
