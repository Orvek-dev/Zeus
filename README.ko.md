...
플랫폼 — Claude Code와 hermes-agent는 현재 라이브 도그푸딩 중이고,
OpenClaw는 어댑터 계약 동결·실호스트 검증 대기 — 이 그대로 수행하고, 그
플랫폼이 관문(게이트)을 통해 Zeus에 연결됩니다. Zeus는 무엇이 실행되어도
되는지 결정하고, 실제로 무슨 일이 있었는지 기록하고, 일어나서는 안 되는
일을 차단하며, 깨끗한 실적을 "덜 묻는 자율성"으로 바꿔 줍니다.
...
- **자격증명 브로커** — 에이전트는 비밀 참조로만 계획하고 원시 키를 절대
  ...
  사실 자체가 실행 결과로 기록됩니다.
- **자기 보호 티어** — Zeus 컨트롤 플레인 자료, 훅 설정, 브로커 참조,
  페어링 상태는 일반 그랜트보다 위에 있습니다. 넓은 `fs.write` 그랜트도
  이 티어를 덮을 수 없고, `zeus tripwire`가 훅을 거치지 않은 외부 변경을
  잡아냅니다.
- **완료 게이트** — Stop/post-task의 "끝났다"는 주장은 결정론적 증거로
  확인할 수 있습니다. 선언한 산출물은 실제로 존재해야 하고, 선언한 테스트
  명령은 실행 증거가 있어야 합니다.
- **오퍼레이터 인박스** — 대기 중인 ask는 짧은 ID, 공통 카드 형식,
  `zeus approvals --pending`, `zeus approve --last --confirm`, 웹훅 전달,
  사고 시 freeze 스위치로 관리합니다.
...
| 관문 | 하는 일 | 상태 |
| --- | --- | --- |
| **게이트 0 — 호스트 훅** | Claude Code PreToolUse → `decide()`, PostToolUse → `record()`. Hermes 훅 온보딩은 `zeus connect hermes --check`로 확인합니다. 정적 도구→능력 매핑, 셸 명령은 결정적 위험 분류, 다섯 답 + 단계별 응답의 승인 카드. | Claude Code + Hermes 라이브 도그푸딩 |
...
| **게이트 3 — zeusd Decision API** | 프록시 포트 위의 `POST /zeus/decide·record`, `GET /zeus/brief`. HMAC 페어링 — 무확인 연결은 절대 없음. hermes: 블로킹 `pre_tool_call` + 감쇠된 자식 주체(봉투 밖 서브에이전트 = DENY) + canary 영수증 확인. OpenClaw: 내구성 있는 TTL fail-closed park의 exec 승인 릴레이. | 구현 완료, 고정 실호스트 95% 소크 게이트는 아직 대기 |
...
호스트가 행동을 park하면, 컨트롤 플레인에서 해결한 뒤 같은 호스트 행동을
다시 실행합니다:

```sh
zeus approvals --pending
zeus approve --last --confirm
zeus approve --parked <짧은-id> --narrow-path /work/project
```
...
**6. 도그푸딩에 필요한 오퍼레이터 안전장치 추가:**

```sh
zeus tripwire --snapshot       # 컨트롤 플레인 파일 해시 기준선
zeus tripwire --check          # 훅 밖에서 생긴 변경 감지
zeus notify --webhook <url>    # 대기 중인 승인 카드를 웹훅으로 전달
zeus freeze --reason incident  # --release 전까지 새 결정 거부
zeus latency --samples 50      # warm 결정 지연 예산 측정
```
...
zeus connect hermes --check --port 8788        # 훅 canary + 영수증 확인
```
...
| 자기 보호 | 컨트롤 플레인 파일, 훅 설정, 브로커 참조, 페어링 상태는 일반 그랜트보다 위에서 보호되며, tripwire 스냅샷이 외부 변경을 잡아낸다. |
...
로컬 도그푸딩/eval 하네스는 의도적으로 비공개·ignored 상태로 유지하며,
제품 동작, 공개 테스트, 분리 경계 규칙만 커밋합니다.
