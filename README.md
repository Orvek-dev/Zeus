...
<p align="center">
  <a href="https://github.com/Orvek-dev/Zeus/releases/tag/v1.1.0"><img alt="Version" src="https://img.shields.io/badge/version-1.1.0-2ea44f"></a>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-1336%20passed-1f883d">
...
an unconstrained chat loop. The public `v1.1.0` source release builds on the
stable governed live platform boundary with Real Provider Runtime: provider
profiles, governed local deterministic provider smoke, controlled external
provider receipt validation, explicit opt-in, endpoint allowlisting, scoped
secret references, budget/timeout gates, audit, redaction, and
no-production-claim controls.
...
`v1.1.0` includes Real Provider Runtime on top of the public MCP Owned Client
Live checkpoint. It proves provider profile reporting, local deterministic
provider smoke, controlled external provider receipt validation, explicit
...
| Package build | Editable install, sdist, and wheel build for `zeus-agent==1.1.0` | passed |
...
`v1.1.0` is the first post-stable governed live platform expansion. The
...
| [Security policy](SECURITY.md) | Public security posture and current v1.1.0 boundary |
