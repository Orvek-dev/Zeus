...
<p align="center">
  <a href="https://github.com/Orvek-dev/Zeus/releases/tag/v0.8.0"><img alt="Version" src="https://img.shields.io/badge/version-0.8.0-2ea44f"></a>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-1247%20passed-1f883d">
...
an unconstrained chat loop. The public `v0.8.0` source checkpoint advances the
live spine into Platform Surface: CLI, API, gateway, ACP, batch, and Python
library entrypoints are represented as governed dry-run contracts with
loopback-default posture, review gates, approval leases, no-secret-echo checks,
...
zeus release-gated-ulw --target-version v0.8.0 --json
...
| Runtime breadth | Mature live platform with many providers, tools, toolsets, gateways, terminal/browser/web/MCP backends, memory, skills, and cron | Public v0.8.0 Platform Surface checkpoint with deterministic CLI/API/gateway/ACP/batch/library entrypoint contracts, Tool Limbs, native tool catalog, MCP discovery contract, API connector contract, provider/MCP loopback readiness, release-gated authority/lease evidence, total architecture contracts, and Zeus Core Language |
...
`v0.8.0` includes the public Platform Surface checkpoint for attaching real
...
contracts before live execution is enabled.
...
| --- | --- | --- |
| Unit and scenario tests | Kernel, objective, provider, tool, transport, workflow, gateway/API, live loop, MCP manager, tool sandbox, research provider, observability, verification, skill-evolution, release-gated ULW, Tool Limbs, Platform Surface, core language, release version, public docs hygiene, and total architecture surfaces | `1247` public tests passed |
...
| Package build | Editable install, sdist, and wheel build for `zeus-agent==0.8.0` | passed |
...
`v0.8.0` is a governed Platform Surface source checkpoint. The supported public
surface is:

- local deterministic CLI scenarios through `zeus`;
- `release-gated-ulw --target-version v0.8.0 --json` for the sequential
...
| [Security policy](SECURITY.md) | Public security posture and current v0.8.0 boundary |
