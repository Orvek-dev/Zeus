...
<p align="center">
  <a href="https://github.com/Orvek-dev/Zeus/releases/tag/v1.0.0-rc.3"><img alt="Version" src="https://img.shields.io/badge/version-1.0.0--rc.3-2ea44f"></a>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-1282%20passed-1f883d">
...
an unconstrained chat loop. The public `v1.0.0-rc.3` source checkpoint adds the
MCP Live Server surface on top of Provider Live API: a governed loopback MCP
HTTP smoke path that composes MCP catalog provenance, activation policy,
runtime lease, credential binding, secret material proof, request envelope,
execution authorization, audit, response redaction, prompt-injection scan, and
cleanup without claiming remote MCP, resources/prompts, external non-loopback,
or production live execution. Gateway delivery, browser control, terminal
execution, remote runtimes, and production external AI APIs should still be
...
zeus release-gated-ulw --target-version v1.0.0-rc.3 --json
...
| Runtime breadth | Mature live platform with many providers, tools, toolsets, gateways, terminal/browser/web/MCP backends, memory, skills, and cron | Public v1.0.0-rc.3 MCP Live Server checkpoint with deterministic CLI/API/gateway/ACP/batch/library entrypoint contracts, Tool Limbs, native tool catalog, MCP discovery contract, API connector contract, local MemoryGraph, LLM Wiki, ontology review queue, skill-learning memory bridge, adaptive workflow pattern selection, critique checkpoints, live readiness, opt-in smoke, live cockpit, provider/MCP/gateway beta contracts, identity/auth/approval/lease/credential/secret/audit/sandbox controls, production foundation contracts, loopback provider HTTP smoke, loopback MCP HTTP smoke, MCP prompt-injection scan, release-gated authority/lease evidence, total architecture contracts, and Zeus Core Language |
...
`v1.0.0-rc.3` includes the public MCP Live Server checkpoint for attaching MCP
servers through a governed contract. It proves a controlled loopback MCP HTTP
smoke path with catalog provenance, activation policy, runtime lease,
credential binding, secret material proof, request envelope, execution
authorization, audit, redaction, prompt-injection scan, and cleanup. Native
tools, gateway delivery, web research, browser or terminal automation, remote
sandboxes, remote MCP servers, and production external AI APIs remain future
live surfaces that must pass through entrypoint contracts, adaptive workflow
selection, live readiness, opt-in smoke, identity/auth controls, runtime lease,
credential binding, secret resolver, audit, sandbox, rollback, review, and
retention/promotion boundaries before production live execution is enabled.
...
| Package build | Editable install, sdist, and wheel build for `zeus-agent==1.0.0rc3` | passed |
...
## v1.0.0-rc.3 Readiness

`v1.0.0-rc.3` is a governed MCP Live Server source checkpoint. The
supported public surface is:

- local deterministic CLI scenarios through `zeus`;
- `release-gated-ulw --target-version v1.0.0-rc.3 --json` for the sequential
  v0.6.0 -> v1.0.0-rc.3 release-gate program contract;
...
| [Security policy](SECURITY.md) | Public security posture and current v1.0.0-rc.3 boundary |
