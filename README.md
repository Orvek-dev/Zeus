...
<p align="center">
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-1324%20passed-1f883d">
...
an unconstrained chat loop. The public `v1.0.0` source release stabilizes the
governed live platform boundary: Provider Live API, MCP Live Server, Gateway
Live Delivery, Sandbox Terminal Live, Memory Privacy Live, Provider Live
Opt-in, Provider Owned Client Live, MCP Owned Client Live, and the
`stable-release` contract are all wired through explicit objective, authority,
lease, secret, audit, redaction, evidence, and no-production-claim controls.
...
`v1.0.0` includes the public MCP Owned Client Live checkpoint for governed
...
`v1.0.0` is the stable governed live platform source release. The
