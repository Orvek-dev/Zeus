...
The public `v4.1.0` code does not claim all live surfaces are
...
| Live capability | Mature live ecosystem with many providers, gateways, tools, MCP, cron, and terminal/browser backends | v4.1.0 is a governed local-first platform boundary with Goal Intelligence, Cognitive Provider Activation, ObjectiveRun start/status/export, productized status, provider/MCP/gateway/runtime contracts, memory/ontology surfaces, self-evolution review queues, loopback/dry-run/live-beta reporting, and stabilized Zeus Core Language; production live integrations remain behind the same authority, lease, approval, sandbox, audit, evidence, rollback, and promotion boundary |
...
- governed Goal Intelligence Platform reporting for structured intent frames,
  ...
  auto-promotion;
- governed Installable Live Platform reporting for provider, MCP, gateway, API,
  CLI, Python library, plugin manifest, local sandbox policy, and remote sandbox
  policy install surfaces while keeping plugins quarantined and remote sandbox
  execution blocked by default;
- governed Production Scale Platform reporting for plugin ecosystem quarantine,
...
- governed Cognitive Provider Activation reporting that routes Goal
...
- governed ObjectiveRun reporting for persisted local run state, goal
  contracts, current plans, evidence records, and evidence-based completion
  summaries;
...
- current public tests, a 10/10 final architecture eval, and a 9/9 total
  architecture eval;
- release-gated ULW status for the v0.6.0 -> v4.1.0 program with
  v4.1.0 objective execution spine checkpoint reporting;
