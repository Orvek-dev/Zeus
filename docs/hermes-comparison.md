...
The public `v1.8.0` code does not claim all live surfaces are production-active.
...
Real MCP Runtime boundary, Real Platform Runtime boundary, Real Execution Runtime boundary, Real Memory Operation Runtime boundary, Real Self Evolution Runtime boundary, Real Product Platform Runtime boundary, Zeus Identity Activation Runtime boundary, and live connection design those
...
- governed Real Self Evolution Runtime reporting for eval-learning smoke,
  reviewable skill proposals, workflow critique memory, promotion block,
  secret-boundary checks, and no active skill/rule or workflow-pattern
  auto-promotion;
- governed Real Product Platform Runtime reporting for product-facing persona,
...
- CLI eval surfaces;
- 1405 public tests, a 10/10 final architecture eval, and a 9/9 total
  architecture eval;
- release-gated ULW status for the v0.6.0 -> v1.8.0 program with
  v1.8.0 Zeus identity activation checkpoint reporting;
