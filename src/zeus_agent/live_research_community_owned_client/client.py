...
class CommunityResearchOwnedClient:
    def __init__(
        self,
        *,
        fetcher: CommunityResearchHttpFetcher,
        endpoint: str,
    ...
    def search(self, request: LiveResearchOwnedClientRequest) -> LiveResearchOwnedClientReceipt:
        if request.source_id != "community":
            return LiveResearchOwnedClientReceipt(
                status_code=400,
                latency_ms=0,
                source_pin_ref=request.source_pin_ref,
                result_count=0,
                response_payload={"items": [], "error": "community_source_required"},
