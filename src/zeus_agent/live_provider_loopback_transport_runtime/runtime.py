...
LiveProviderLoopbackDecision = Literal["executed", "blocked"]
ProviderTransportKind = Literal["loopback_http", "external_http"]

_MODEL_CONFIG: Final = ConfigDict(extra="forbid", frozen=True, hide_input_in_errors=True)
_TRANSPORT_KINDS: Final = frozenset(("loopback_http", "external_http"))
_CLEANUP_RECEIPT: Final = "provider-loopback-no-network"
...
class LiveProviderLoopbackTransportResult(BaseModel):
    model_config = _MODEL_CONFIG

    decision: LiveProviderLoopbackDecision
    execution_id: Optional[str]
    activation_id: Optional[str]
    adapter_plan_id: Optional[str]
    request_envelope_id: Optional[str]
    transport_kind: Optional[ProviderTransportKind]
...
class LiveProviderLoopbackTransportRuntime:
    def execute(
        self,
        *,
        ...
        reasons.extend(_adapter_plan_reasons(adapter_plan=adapter_plan, provider_envelope=provider_envelope))
        reasons.extend(_provider_envelope_reasons(provider_envelope))
        if safe_transport_kind not in _TRANSPORT_KINDS:
            reasons.append("unsupported_provider_transport_kind")
        if safe_transport_kind == "external_http":
            reasons.append("provider_external_http_not_implemented")
        ...
        return _result(
            ...
            response=_loopback_response(provider_envelope),
        )
...
def _activation_reasons(
    *,
    ...
    if activation.adapter_kind != "provider":
        reasons.append("transport_activation_not_provider")
...
def _execution_id(
    *,
    ...
    return "live-provider-loopback-{0}".format(hashlib.sha256(encoded).hexdigest()[:16])


def _loopback_response(provider_envelope: LiveProviderRequestResult) -> dict[str, JsonValue]:
    return {
        "mode": "loopback_http",
        "provider_kind": provider_envelope.provider_kind,
        "model_id": provider_envelope.model_id,
        "message_digest": provider_envelope.message_digest,
...
def _result(
    *,
    ...
    kind: Optional[ProviderTransportKind] = transport_kind if transport_kind in _TRANSPORT_KINDS else None
    result = LiveProviderLoopbackTransportResult(
        decision=decision,
...
def _no_secret_echo(result: LiveProviderLoopbackTransportResult) -> bool:
