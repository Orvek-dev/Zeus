...
def register_wave97_commands(app: typer.Typer) -> None:
    @app.command("live-provider-adapter-plan")
    def live_provider_adapter_plan(
        ...
        try:
            ...
            envelope = LiveProviderRequestResult.model_validate_json(provider_envelope_json)
        except ValidationError as exc:
            _print_payload(
                {
                    "decision": "blocked",
                    "reason": "malformed_live_provider_adapter_plan",
                    "error": str(exc),
