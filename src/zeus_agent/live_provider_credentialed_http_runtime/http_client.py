...
from zeus_agent.live_provider_owned_client_transport_runtime.response import redact_response

_LOOPBACK_HOSTS: Final = frozenset(("127.0.0.1", "localhost", "::1"))


@dataclass(frozen=True)
class ProviderCredentialedHttpClientResult:
    status_code: int
    latency_ms: int
...
def provider_endpoint_host(endpoint: str) -> Optional[str]:
    parsed = urlparse(endpoint)
    return parsed.hostname


def post_provider_body(
    *,
    endpoint: str,
    header_name: str,
    header_value: str,
    ...
    timeout_ms: int,
) -> ProviderCredentialedHttpClientResult:
    ...
    return ProviderCredentialedHttpClientResult(
