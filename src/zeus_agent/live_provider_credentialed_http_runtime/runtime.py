...
from zeus_agent.live_provider_credentialed_http_runtime.http_client import (
    post_provider_body,
    provider_endpoint_host,
)
from zeus_agent.live_provider_credentialed_http_runtime.models import (
    LiveProviderCredentialedHttpDecision,
    LiveProviderCredentialedHttpResult,
)
from zeus_agent.live_provider_credentialed_http_runtime.rules import (
    binding_reasons,
    injection_reasons,
    provider_envelope_reasons,
    request_body_reasons,
    safe_optional,
    transport_reasons,
)
from zeus_agent.live_provider_credentialed_http_runtime.sealed_capture import CredentialCapture
from zeus_agent.live_provider_owned_client_transport_runtime.response import no_secret_echo
...
_CLEANUP_RECEIPT: Final = "provider-credentialed-http-client-closed"


class LiveProviderCredentialedHttpRuntime:
    def execute(
        self,
        *,
        ...
        reasons.extend(provider_envelope_reasons(provider_envelope))
        reasons.extend(request_body_reasons(request_body))
        reasons.extend(binding_reasons(injection, provider_envelope, request_body))
        ...
        consumer = CredentialCapture("provider-credentialed-http://consumer")
        ...
        try:
            client_result = post_provider_body(
            ...
            return _blocked_after_network(
                ...
                safe_execution_ref,
                release.release_id,
                "provider_credentialed_http_status_error",
                exc.code,
            )
            ...
            return _blocked_after_network(
                ...
                safe_execution_ref,
                release.release_id,
                "provider_credentialed_http_transport_error",
                None,
            )
        if client_result.redacted_payload is None:
            return _blocked_after_network(
                ...
                safe_execution_ref,
                release.release_id,
                "provider_credentialed_http_response_not_json_object",
        ...
        return _result(
            ...
            execution_ref=safe_execution_ref,
            release_id=release.release_id,
            execution_id=_execution_id(injection, provider_envelope, request_body, safe_endpoint, safe_execution_ref),
...
def _execution_id(
    ...
    return "live-provider-credentialed-http-{0}".format(hashlib.sha256(encoded).hexdigest()[:16])
...
def _result(
    ...
    **flags: bool,
    ...
    result = LiveProviderCredentialedHttpResult(
        decision=decision,
        ...
        request_envelope_id=None if provider_envelope is None else provider_envelope.request_envelope_id,
        body_id=None if request_body is None else request_body.body_id,
        provider_endpoint=None if provider_envelope is None else provider_envelope.endpoint,
        transport_endpoint=transport_endpoint,
        transport_endpoint_host=None if transport_endpoint is None else provider_endpoint_host(transport_endpoint),
        execution_ref=execution_ref,
        release_ref=release_ref,
