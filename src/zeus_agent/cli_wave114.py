...
def register_wave114_commands(app: typer.Typer) -> None:
    @app.command("live-gateway-external-transport")
    def live_gateway_external_transport(
        ...
        try:
            ...
            client_result = LiveGatewayExternalClientResult.model_validate_json(client_result_json)
        except ValidationError as exc:
            _print_payload(
                {
                    "decision": "blocked",
                    "reason": "malformed_live_gateway_external_transport",
                    "error": str(exc),
