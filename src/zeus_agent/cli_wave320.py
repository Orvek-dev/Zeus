...
def register_wave320_commands(app: typer.Typer) -> None:
    @app.command("installable-live-platform")
    def installable_live_platform(
        scenario: str = typer.Option("status", "--scenario"),
        home: Optional[Path] = typer.Option(None, "--home"),
        operator_note: Optional[str] = typer.Option(None, "--operator-note"),
        as_json: bool = typer.Option(False, "--json"),
    ) -> None:
        payload = build_installable_live_platform_contract(
            scenario=scenario,
            home=home,
