...
from zeus_agent.live_gateway_delivery_adapter_runtime.rules import (
    claim_reasons,
    client_request,
    preflight_reasons,
    receipt_reasons,
...
LiveGatewayDeliveryAdapterDecision = Literal["executed", "blocked"]

_CLEANUP_RECEIPT: Final = "gateway-delivery-adapter-client-closed"


class LiveGatewayDeliveryAdapterRuntime:
    def execute(
        self,
        *,
        ...
        client: LiveGatewayDeliveryAdapterClient,
        execution_ref: str,
    ) -> LiveGatewayDeliveryAdapterResult:
        safe_ref = _safe_optional(execution_ref)
        reasons = list(claim_reasons(claim))
        reasons.extend(preflight_reasons(policy, preflight, handoff, gateway_envelope))
        ...
        request = None if reasons else client_request(claim, preflight, credential_injection, gateway_envelope)
        if request is None:
            return _result(
                decision="blocked",
                claim=claim,
        ...
        return _result(
            ...
            gateway_envelope=gateway_envelope,
            execution_ref=safe_ref,
            execution_id=_execution_id(claim, policy, preflight, handoff, credential_injection, gateway_envelope, safe_ref),
            receipt=receipt,
            gateway_delivery_adapter=True,
            production_claim_bound=True,
            credential_injection_bound=True,
            request_constructed=True,
            header_value_ref_bound=True,
...
def _execution_id(
    ...
    return "live-gateway-delivery-adapter-{0}".format(hashlib.sha256(encoded).hexdigest()[:16])


def _result(
    *,
    decision: LiveGatewayDeliveryAdapterDecision,
    claim: Optional[LiveProductionClaimResult],
    policy: Optional[LiveRemoteTransportPolicyResult],
    preflight: Optional[LiveRemoteExecutorPreflightResult],
    handoff: Optional[LiveRemoteCredentialHandoffResult],
    ...
    receipt: Optional[LiveGatewayDeliveryAdapterReceipt] = None,
    **flags: bool,
) -> LiveGatewayDeliveryAdapterResult:
    result = LiveGatewayDeliveryAdapterResult(
        decision=decision,
