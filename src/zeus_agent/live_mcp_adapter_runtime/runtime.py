...
LiveMcpAdapterDecision = Literal["planned", "blocked"]
McpTransportMode = Literal["dry_run", "live"]
...
class LiveMcpAdapterResult(BaseModel):
    model_config = _MODEL_CONFIG

    decision: LiveMcpAdapterDecision
    ...
    transport_mode: Optional[McpTransportMode]
    timeout_ms: int
    retry_attempts: int
    idempotency_key: str
...
def _release_reasons(
    *,
    ...
    if release.executor_kind != "mcp":
        reasons.append("executor_kind_not_mcp")
    if release.envelope_id != mcp_envelope.request_envelope_id:
...
def _mcp_envelope_reasons(mcp_envelope: LiveMcpRequestResult) -> tuple[str, ...]:
    ...
    if not mcp_envelope.secret_material_verified or not mcp_envelope.tool_allowlisted:
        reasons.append("mcp_tool_not_ready")
...
def _policy_reasons(
    ...
    if transport_mode == "live":
        reasons.append("mcp_live_transport_not_implemented")
...
def _adapter_plan_id(
    *,
    ...
    return "live-mcp-adapter-plan-{0}".format(hashlib.sha256(encoded).hexdigest()[:16])
...
def _result(
    *,
    ...
    mode: Optional[McpTransportMode] = transport_mode if transport_mode in _TRANSPORT_MODES else None
    result = LiveMcpAdapterResult(
        decision=decision,
...
def _no_secret_echo(result: LiveMcpAdapterResult) -> bool:
